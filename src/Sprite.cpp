...
void Sprite::Open (const std::string& file) {
	...
	int status = SDL_QueryTexture(this->texture.get(), nullptr, nullptr, &this->width, &this->height);
	if (status != 0) {
...
void Sprite::Reopen (const std::string& file) {
	this->texture = Resources::GetImage(file);
	int newW, newH;
	int status = SDL_QueryTexture(this->texture.get(), nullptr, nullptr, &newW, &newH);
	if (status != 0) {
		std::string error = IMG_GetError();
		throw std::runtime_error(error);
	}
